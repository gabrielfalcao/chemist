...
from chemist import InvalidColumnName, InvalidQueryModifier, Manager, Model
...
def test_manager_find_one_by():
    ...
    manager.query_by.assert_called_once_with(keyword_one="first", keyword_two="second")
...
def test_manager_find_by(partial):
    ...
    Models_mock.assert_has_calls(
        [call("one record"), call("two record"), call("three record")]
    )
...
def test_manager_all():
    ...
    manager.find_by.assert_called_once_with(
        limit_by=None, offset_by=None, order_by=None
    )
...
def test_manager_all_with_order_by():
    ...
    manager.all(offset_by=100, limit_by=20, order_by="some_field").should.equal(
        "the result"
    )
    manager.find_by.assert_called_once_with(
        limit_by=20, offset_by=100, order_by="some_field"
    )
...
def test_query_by_invalid_column():
    ("Calling Manager#query_by with an invalid field should cause an " "exception")
    ...
    ).should.throw(InvalidColumnName)
...
def test_query_by_invalid_query_modifier():
    ...
    ).should.throw(InvalidQueryModifier)
...
def test_many_from_result_proxy(from_result_proxy):
    ...
    context_mock = MagicMock(name="engine")
    connection = context_mock.engine.begin.return_value.__enter__.return_value
    ...
    manager = Manager(model, context_mock)
...
def test_many_from_query(many_from_result_proxy):
    (
    ...
    context_mock.engine.begin.assert_called_once_with()
    many_from_result_proxy.assert_called_once_with(proxy)


@patch("chemist.managers.Manager.get_connection")
@patch("chemist.managers.Manager.from_result_proxy")
def test_one_from_query(from_result_proxy, get_connection):
    ("Manager#one_from_query should execute the given " "query and return the result")
