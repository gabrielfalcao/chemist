...
def test_user_signup(context):
    ("User.create(dict) should create a " "user in the database")
    data = {
        "username": "octocat",
...
def test_user_all(context):
    ...
    User.total_rows().should.equal(1)
...
def test_user_signup_get_or_create_if_already_exists(context):
    ("User.get_or_create(dict) should get" "user from the database if already exists")
...
def test_query_by_invalid_column(context):
    ("User.query_by(**kwargs) should raise an exception if the field does not" " exist")
